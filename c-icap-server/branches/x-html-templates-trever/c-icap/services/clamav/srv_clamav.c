...
void generate_error_page(av_req_data_t * data, request_t * req)
{
     ...
     error_page =
         templateBuildContent(req, "AVSCAN", "NONVIRALATOR_UNSAFE",
                           AVtemplateConvert);
...
void dbreload_command(char *name, int type, char **argv)
{
     ...
     if (srv_clamav_xdata)
          set_istag(srv_clamav_xdata);
     // Free all htmlTemplates so that we are good. Since we can't free only clamav related,
     // minimal performance hit. All htmlTemplate users should do this onreload.
     templateReload();
}
